...
public class VisualForcePushProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (VisualForcePushProcedure.bufferBuilder == null) {
			if (update)
				clear();
			if (VisualForcePushProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					VisualForcePushProcedure.mode = mode;
					VisualForcePushProcedure.format = format;
					VisualForcePushProcedure.bufferBuilder = Tesselator.getInstance().begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					VisualForcePushProcedure.mode = mode;
					VisualForcePushProcedure.format = format;
					VisualForcePushProcedure.bufferBuilder = Tesselator.getInstance().begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		VisualForcePushProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			VisualForcePushProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, Entity entity) {
		...
		if (entity.getData(StarwarsverseModVariables.PLAYER_VARIABLES).force_indicator) {
			if (entity.getData(StarwarsverseModVariables.PLAYER_VARIABLES).is_using_force) {
				if ((entity.getData(StarwarsverseModVariables.PLAYER_VARIABLES).current_charging_power).equals("force_push")) {
					...
					if (entity.isShiftKeyDown()) {
						...
						if (target(2)) {
							renderShape(shape(), (entity.getX()), (entity.getY() + 0.2), (entity.getZ()), 0, 0, 0, (float) ((20 * (entity.getData(StarwarsverseModVariables.PLAYER_VARIABLES).charge_timer / maxcharge)) / 1), 0,
									(float) ((20 * (entity.getData(StarwarsverseModVariables.PLAYER_VARIABLES).charge_timer / maxcharge)) / 1), 55 << 24 | 0 << 16 | 178 << 8 | 255);
							release();
						}
						clear();
					} else {
