...
public class GuardOverlayProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			GuardOverlayProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				GuardOverlayProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		...
		if (Minecraft.getInstance().player != null) {
			...
			if (entity.getData(StarwarsverseModVariables.PLAYER_VARIABLES).guard > 0) {
				if (entity.getData(StarwarsverseModVariables.PLAYER_VARIABLES).guard < 20) {
					R = 245;
					G = 149;
					B = 5;
				} else if (entity.getData(StarwarsverseModVariables.PLAYER_VARIABLES).guard < 60) {
					R = 32;
					G = 90;
					B = 171;
				} else {
				...
				if (target(1)) {
					renderShape(GuardRingShapeProcedure.execute(), (Minecraft.getInstance().getWindow().getGuiScaledWidth() / 2 - 0), (Minecraft.getInstance().getWindow().getGuiScaledHeight() / 2 - 0), 0, 0, 0, 0,
							(float) (15 + 5 * Minecraft.getInstance().getWindow().getGuiScale()), (float) (15 + 5 * Minecraft.getInstance().getWindow().getGuiScale()), (float) (15 + 5 * Minecraft.getInstance().getWindow().getGuiScale()),
